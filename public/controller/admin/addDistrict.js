...
app.controller("myCtrl", async ($scope, $http) => {
    ...
    $scope.validateDistrict = function () {
        $http.get("http://localhost:7900/bloodDonation/adminRoute/validateDistrict?newDistrictName=" + $scope.newDistrictName)
    ...
    $scope.addDistrict = async () => {

        $scope.data1 = [];
        $scope.data = {};
        let districtName = $scope.newDistrictName;
        let samitiName =$scope.newSamitiName;
       
        $scope.data.newDistrictName = districtName.charAt(0).toUpperCase() + districtName.slice(1);
        $scope.data.newSamitiName = samitiName.charAt(0).toUpperCase() + samitiName.slice(1);
        try {
            await $http.post('http://localhost:7900/bloodDonation/adminRoute/addNewDistrict', $scope.data);
            window.alert(`New District added Sucessfully`);
            window.location.href = "http://localhost:7900/bloodDonation/admin/addDistrict";
        } catch (e) {
