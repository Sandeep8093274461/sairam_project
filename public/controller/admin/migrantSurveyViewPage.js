...
app.controller("myCtrl", async ($scope, $http) => {
    ...
    $scope.allDataShow = function (y) {
        try {
            $http.get('http://localhost:7900/bloodDonation/adminRoute/allMigrantDatashow?migrate_id=' + migrate_id)
                .then(function (response) {
                    $scope.alldata = response.data.details;
                    $scope.allskillData = response.data.skilldatalist;
                });
...
function loadPage(content, callback) {
    ...
    mywindow.document.write('<html><head><title>Survey for Promotion of  Sai Swabalambi</title><meta name="user_id" content="print">');
