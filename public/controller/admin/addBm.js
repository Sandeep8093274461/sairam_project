...
app.controller("myCtrl", async ($scope, $http) => {
    ...
    $scope.loadSamithiList = async () => {
        try {
            let response = await $http.get('http://localhost:7900/bloodDonation/adminRoute/getresoursesDistWiseSamithiList?dist_id=' + $scope.dist.district_id)
            $scope.samithiList = response.data;
            $scope.$apply();
        } catch (e) { }
    }
    $scope.validateBm = function () {
        $http.get("http://localhost:7900/bloodDonation/adminRoute/validateBm?newBmName=" + $scope.newBmName+ "&samithi_id=" + $scope.samithi.samithi_id)
            .then(function (response) {
                if (response.data == "") {
                    $scope.bmId = false;
                }
    ...
    $scope.addBm = async () => {
        ...
        $scope.data1 = [];
        $scope.data = {};
        $scope.data.district_name = $scope.dist.name;
        $scope.data.district_id = $scope.dist.district_id;
        $scope.data.samithi_name = $scope.samithi.name;
        $scope.data.samithi_id = $scope.samithi.samithi_id;
        $scope.data.newBmName = $scope.newBmName;
        $scope.data.newRegdno = $scope.newRegdno;
        try {
            await $http.post('http://localhost:7900/bloodDonation/adminRoute/addNewBm', $scope.data);
            window.alert(`New Samiti added Sucessfully`);
            window.location.href = "http://localhost:7900/bloodDonation/admin/addBm";
        } catch (e) {
