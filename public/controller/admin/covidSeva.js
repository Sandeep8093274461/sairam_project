...
app.controller("myCtrl", async ($scope, $http) => {
    ...
    $scope.loadDistricts = function () {
        $http.get('http://localhost:7900/bloodDonation/adminRoute/getConvenordatadistdistinct')
            .then(response => {
                ...
                $http.get('http://localhost:7900/bloodDonation/adminRoute/getConvenordatadist')
                    .then(response2 => {
                        $scope.ConvenorList = response2.data;
                    })
                if (qs["id"] != null) {
                    $http.get('http://localhost:7900/bloodDonation/adminRoute/getConvenorDetailsId/' + qs["id"])
                        .then(response1 => {
                            var resCm = response1.data;
                            $http.get('http://localhost:7900/bloodDonation/adminRoute/getConvenordatadist/' + resCm[0].District)
                                .then(response4 => {
                            ...
                            for (let imgd of imglink) {
                                ...
                                aTag.setAttribute('href', "http://localhost:7900/bloodDonation/adminRoute/downloadFile/" + JSON.parse(imglink[j]).campFile);
    ...
    $scope.edit = function () {
        if ($scope.conv_id != '') {
            $http.get("http://localhost:7900/bloodDonation/adminRoute/convenorDetailsData?conv_id=" + $scope.conv_id)
                .then(function (response) {

                    $scope.details = response.data[0];
                    $scope.date = new Date($scope.details.date)
                    // $scope.date = new Date(Date.parse($scope.details.date)+ 106199999);
                    $scope.SevaSamithi = $scope.details.samiti;
                    $scope.Address = $scope.details.location;
                    $scope.nfp = $scope.details.nfp;
                    $scope.nak = $scope.details.nfk;
                    $scope.othitems = parseInt($scope.details.other_items);
                    $scope.bdos = $scope.details.detail_of_seva;
                    $scope.nsi = $scope.details.nosdi;
                    $scope.aci = $scope.details.app_cost;
                    $scope.photo2 = $scope.details.photo2;
                    ...
                    $http.get("http://localhost:7900/bloodDonation/adminRoute/getresoursesdistwisedata")
                        .then(function (response) {
                            $scope.districtList = response.data;
                            $scope.dist = $scope.details.district_id;
    ...
    $scope.loadSamithiList = async () => {
        try {
            let response = await $http.get('http://localhost:7900/bloodDonation/adminRoute/getDistWiseSamithiList/' + $scope.dist)
            $scope.samithiList = response.data;
            $scope.$apply();
        } catch (e) { }
    }
            ...
            if (theFile != undefined) {
                ...
                if (checkType !== "image/jpeg" && checkType !== "image/png") {
                    ...
                    let data = {
                        ...
                        'conv_id': parseInt($scope.conv_id)
                    ...
                    try {
                        ...
                        await $http.post('http://localhost:7900/bloodDonation/adminRoute/changeMyUpdatesDB1', fd, {
                            transformRequest: angular.identity,
                            headers: { 'Content-Type': undefined },
                            body: { data }
                        });
                        window.alert(`Convener is updated successfully.`);
                ...
                var fd = new FormData();
                let data = {
                    // 'condition': { 'convId': parseInt($scope.conv_id) },
                    // 'newValue': {
                    'Date': $scope.date,
                    'dist_id': $scope.dist,
                    'SevaSamithi': $scope.SevaSamithi,
            ...
            if (theFile != undefined) {
                ...
                if (checkType !== "image/jpeg" && checkType !== "image/png") {
                    ...
                    // validation according to file size(in bytes)
                    {
                        ...
                        try {
                            await $http.post('http://localhost:7900/bloodDonation/adminRoute/addConvenor', fd, {
                                transformRequest: angular.identity,
                                headers: { 'Content-Type': undefined }
                            });
    ...
    $scope.bindDetailsData = async () => {
        ...
        $http.get('http://localhost:7900/bloodDonation/adminRoute/getConvenordatadist/' + $scope.dist)
            .then(response2 => {
    ...
    $scope.bindDetailsDataSamiti = async () => {
        ...
        $http.get('http://localhost:7900/bloodDonation/adminRoute/getConvenordataSamiti/' + $scope.SevaSamithi)
            .then(response2 => {
