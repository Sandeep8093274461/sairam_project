...
app.controller("myCtrl", async ($scope, $http) => {
        ...
        if (photo1 != undefined && photo2 != undefined) {
            ...
            request.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    // saiSabujeema
                    window.location.href = "http://localhost:7900/bloodDonation/admin/activitiesReport";
                }
            };
            data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
            request.open('POST', "http://localhost:7900/bloodDonation/adminRoute/addActivitiesReportaData");
            request.send(data);
    ...
    $scope.edit = function () {
        if ($scope.activitiesreportid != '') {
            $http.get("http://localhost:7900/bloodDonation/adminRoute/getActivitiesReportApplicationDetails?activitiesreportid=" + $scope.activitiesreportid)
                .then(function (response) {
    ...
    $scope.update = async () => {
        ...
        request.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                window.location.href = "http://localhost:7900/bloodDonation/admin/activitiesReportAllData";
            }
        };
        data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
        request.open('POST', "http://localhost:7900/bloodDonation/adminRoute/updateActivitiesReportData");
        request.send(data);
