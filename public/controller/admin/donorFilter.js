...
app.controller("myCtrl", async ($scope, $http) => {
    ...
    $scope.loadDonorIdData = function () {
        $http.get("http://localhost:7900/bloodDonation/adminRoute/loadDonerId")
    ...
    $scope.retriveDonorDetails = async () => {
        ...
        $http.get('http://localhost:7900/bloodDonation/adminRoute/getDonorById?donerid=' + $scope.donorSelect)
            .then(response1 => {
    ...
    $scope.donationDetails = async () => {
        ...
        $scope.showdonatedByError = false;
        ...
        if (parseInt(valFlag) == 1) {
            ...
            try {
                await $http.post('http://localhost:7900/bloodDonation/adminRoute/addDonoationDetails', { data: data });
                window.alert(`Donation is added successfully.`);
                window.location.href = 'http://localhost:7900/bloodDonation/admin/donorFilter'
            } catch (e) {
