...
app.controller("myCtrl", async ($scope, $http,Excel, $timeout) => {
    ...
    $scope.loadDistricts();
    $scope.getFilteredData = function (y) {
        if ($scope.fromDate != undefined && $scope.toDate != undefined) {
            $http.post('http://localhost:7900/bloodDonation/adminRoute/getCmpDetails', { dist_id: $scope.dist, fromDate: $scope.fromDate, toDate: $scope.toDate, district_id: district_id, user_id: $scope.user_id })
            .then(response => {
                $scope.indexCampList = response.data;
                $scope.showAllList = true;
                angular.forEach(response.data, element => {
                    element.cmpdate = new Date(Date.parse(element.cmpdate)+ 106199999)
                    
                    if(element.campdateto == null){
                        element.campdateto=element.campdateto;
                    }
                    else{
                        element.campdateto = new Date(Date.parse(element.campdateto)+ 106199999)
                    }
    ...
    $scope.editField = function (y) {
        $scope.campid = y;
        window.location.href = "http://localhost:7900/bloodDonation/admin/campUpdate?campid=" + $scope.campid;
    ...
    $scope.deleteField = async (campid) => {
        try {
            await $http.post('http://localhost:7900/bloodDonation/adminRoute/campUpdateDataDeleted', { campid: campid });
    ...
    $scope.exportToExcel = function (tableIdSeed) {
        var exportHref = Excel.tableToExcel(tableIdSeed, 'campdetails');
