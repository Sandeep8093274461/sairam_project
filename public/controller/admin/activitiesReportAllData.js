...
app.controller("myCtrl", async ($scope, $http, Excel, $timeout) => {
    ...
    $scope.getFilteredData = async () => {
        if ($scope.fromDate != undefined && $scope.toDate != undefined) {
            $http.post('http://localhost:7900/bloodDonation/adminRoute/activitiesReportAllData', { dist_id: $scope.dist, fromDate: $scope.fromDate, toDate: $scope.toDate, district_id: district_id, user_id: $scope.user_id })
                .then(response => {
    ...
    $scope.getFilteredData1 = async () => {
        if ($scope.fromDate != undefined && $scope.toDate != undefined) {
            $http.post('http://localhost:7900/bloodDonation/adminRoute/activitiesReportAllData', { dist_id: $scope.dist, fromDate: $scope.fromDate, toDate: $scope.toDate, district_id: district_id, user_id: $scope.user_id })
                .then(response => {

                    $scope.alldatalist = response.data;
                    $scope.showAllData1 = true;
                    $scope.showAllList = false;
    ...
    $scope.view = function (y) {
        $scope.activitiesreportid = y;
        try {
            $http.get('http://localhost:7900/bloodDonation/adminRoute/activitiesReportDatashow?activitiesreportid=' +$scope.activitiesreportid)
                .then(function (response) {
    ...
    $scope.edit = function (y) {
        $scope.activitiesreportid = y;
        window.location.href = "http://localhost:7900/bloodDonation/admin/activitiesReport?activitiesreportid=" + $scope.activitiesreportid;

    }


    $scope.oneDataDeleted = async (activitiesreportid) => {
        try {
            await $http.post('http://localhost:7900/bloodDonation/adminRoute/activitiesReportDataDeleted', { activitiesreportid: activitiesreportid });
...
function loadPage(content, callback) {
    var mywindow = window.open('', 'Print', 'height=600,width=800,scrollbars=yes ');
    mywindow.document.write('<html><head><title>Activities report</title><meta name="user_id" content="print">');
...
function printElemall() {
    var content = document.getElementById('applicantDetails1').innerHTML;
    loadPage(content, function (mywindow) {
