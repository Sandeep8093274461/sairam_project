...
app.controller("myCtrl", async ($scope, $http,Excel, $timeout) => {
    ...
    $scope.allDataShow = function () {
        try {
            $http.get('http://localhost:7900/bloodDonation/adminRoute/allResourseDataShow?district_id=' + district_id+ "&user_id=" + $scope.user_id)
                .then(function (response) {
    ...
    $scope.view = function (y) {
        $scope.id = y;
        window.location.href = "http://localhost:7900/bloodDonation/admin/resourcePersonsViewPage?id=" + $scope.id;
    }
    $scope.edit = function (y) {
        $scope.id = y;
        window.location.href = "http://localhost:7900/bloodDonation/admin/resourcePersons?id=" + $scope.id;
    ...
    $scope.oneDataDeleted = async (id) => {
        try {
            await $http.post('http://localhost:7900/bloodDonation/adminRoute/oneResourceDataDeleted', { id: id });
            $scope.allDataShow();
