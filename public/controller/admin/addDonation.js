...
app.controller("myCtrl", async ($scope, $http) => {
            ...
            if (donerid != '') {
                ...
                $http.get("http://localhost:7900/bloodDonation/adminRoute/getDonorById?donerid=" + $scope.donerid)
                    .then(function (response1) {
                        ...
                        //     var fromDate = Date.parse(fromdate) + 19800000
                        //     var toDate = Date.parse(todate) + 106199999
                        //     match.issueDate = { $gte: new Date(fromDate), $lte: new Date(toDate) };
                        // } else if ((fromdate != "undefined") && (todate == "undefined")) {
                        //     var fromDate = Date.parse(fromdate) + 19800000
                        //     match.issueDate = { $gte: new Date(fromDate) };
                        // } else if ((fromdate == "undefined") && (todate != "undefined")) {
                        //     var toDate = Date.parse(todate) + 106199999
                        //     match.issueDate = { $lte: new Date(toDate) };
                        // }
                    })
    ...
    $scope.updateData = async () => {
        ...
        if (parseInt(valFlag) == 1) {
            ...
            //     if ($scope.lastDonationDate == null) {

            //         $scope.data.lastDonationDate = '';
            //     }else{
            //         $scope.data.lastDonationDate = $scope.lastDonationDate;
            //     }
            $scope.data1 = [];
            ...
            try {
                await $http.post('http://localhost:7900/bloodDonation/adminRoute/changeMyUpdates1', { updateData: $scope.data, donerid: $scope.donerid, previousLastDonationDate: previousLastDonationDate });
                window.alert(`Updated successfully.`);
                window.location.href = "http://localhost:7900/bloodDonation/admin/detailsDonor";
            } catch (e) {
    ...
    $scope.addData = async () => {
        ...
        if (parseInt(valFlag) == 1) {
            ...
            try {
                await $http.post('http://localhost:7900/bloodDonation/adminRoute/addDonorDetails1', { data: data });
                window.alert(`Donor is added successfully.`);
                window.location.href = "http://localhost:7900/bloodDonation/admin/addDonation";
