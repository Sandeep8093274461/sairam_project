...
app.controller("myCtrl", async ($scope, $http, Excel, $timeout) => {
    ...
    $scope.getFilteredData = async () => {
        if ($scope.fromDate != undefined && $scope.toDate != undefined) {
            $http.post('http://localhost:7900/bloodDonation/adminRoute/beneficiariesReportAllData', { dist_id: $scope.dist, fromDate: $scope.fromDate, toDate: $scope.toDate, district_id: district_id, user_id: $scope.user_id })
                .then(response => {
    ...
    $scope.view = function (y) {
        $scope.beneficiariesreportid = y;
        try {
            $http.get('http://localhost:7900/bloodDonation/adminRoute/beneficiariesReportDatashow?beneficiariesreportid=' + $scope.beneficiariesreportid)
                .then(function (response) {
    ...
    $scope.edit = function (y) {
        $scope.beneficiariesreportid = y;
        window.location.href = "http://localhost:7900/bloodDonation/admin/beneficiariesReport?beneficiariesreportid=" + $scope.beneficiariesreportid;

    }


    $scope.oneDataDeleted = async (beneficiariesreportid) => {
        try {
            await $http.post('http://localhost:7900/bloodDonation/adminRoute/beneficiariesReportDataDeleted', { beneficiariesreportid: beneficiariesreportid });
    ...
    $scope.exportToExcel = function (tableIdSeed) {
        var exportHref = Excel.tableToExcel(tableIdSeed, 'beneficariesReport');
