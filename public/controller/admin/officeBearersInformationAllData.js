...
app.controller("myCtrl", async ($scope, $http, Excel, $timeout) => {
    ...
    $scope.loadFilterlevel = async () => {
        try {
            let response = await $http.get("http://localhost:7900/bloodDonation/adminRoute/getLevel")
            $scope.levelList = response.data;
            $scope.$apply();
        } catch (e) { }

    }
    $scope.loadFilterlevel();
    $scope.loadFilterDistricts = async () => {
        try {
            let response = await $http.get("http://localhost:7900/bloodDonation/adminRoute/getresoursesdistwisedata")
            $scope.districtList = response.data;
            $scope.$apply();
            $scope.loadFilterSamithiList();
        } catch (e) { }

    }
    $scope.loadFilterSamithiList = async () => {
        try {
            let response = await $http.get('http://localhost:7900/bloodDonation/adminRoute/getresoursesDistWiseSamithiList?dist_id=' + $scope.districtFilter)
            $scope.samithiList = response.data;
            $scope.$apply();
            $scope.loadFilterbhajanMandaliList();
        } catch (e) { }
    }
    $scope.loadFilterbhajanMandaliList = async () => {
        try {
            let response = await $http.get('http://localhost:7900/bloodDonation/adminRoute/bhajanMandaliListwiseData?samithi_id=' + $scope.samitiFilter)

            if (response.data.length == 0) {
                $scope.bhajanMandaliListshow = [];
                objectData = $scope.samithiList[$scope.samithiList.findIndex(x => x.samithi_id == $scope.samitiFilter)];
    ...
    $scope.loadDesignationListFilter = async () => {
        try {
            let response = await $http.get('http://localhost:7900/bloodDonation/adminRoute/getDesignationList?lavel_id=' + $scope.levelFilter)
    ...
    //     try {
    //         let response = await $http.get('http://localhost:7900/bloodDonation/adminRoute/getresoursesDistWiseSamithiList?dist_id=' + $scope.fromDistrictFilter)
    //         $scope.samithiList = response.data;
    //         $scope.$apply();
    //     } catch (e) { }
    // }
    ...
    //     try {
    //         let response = await $http.get('http://localhost:7900/bloodDonation/adminRoute/bhajanMandaliListwiseData?samithi_id=' + $scope.fromSamitiFilter)

    //         if (response.data.length == 0) {
    //             $scope.bhajanMandaliListshow = [];
    //             objectData = $scope.samithiList[$scope.samithiList.findIndex(x => x.samithi_id == $scope.fromSamitiFilter)];
    //             $scope.bhajanMandaliListshow.push({ bhajanmandali_name: this.objectData.name, bhajanmandali_id: this.objectData.samithi_id })
    ...
    $scope.getFilteredData = async () => {
        if ($scope.fromDate != undefined && $scope.toDate != undefined) {
            ...
            $http.post('http://localhost:7900/bloodDonation/adminRoute/officeBearersInformationAllData', {level: $scope.levelFilter,designation: $scope.designationFilter,district: $scope.districtFilter,samiti: $scope.samitiFilter,bhajanMandali: $scope.bhajanMandaliFilter, alumni: $scope.alumniFilter, fromDate: $scope.fromDate, toDate: $scope.toDate, district_id: district_id, user_id: $scope.user_id })
                .then(response => {
            ...
            window.alert('Please Select Mandatory Filed');
        }
    ...
    $scope.view = function (y) {
        $scope.officebearersinformationid = y;
        $http.get('http://localhost:7900/bloodDonation/adminRoute/officeBearersInformationshow?officebearersinformationid=' +  $scope.officebearersinformationid)
                .then(function (response) {
                    $scope.alldata1 = response.data.details;
                    $scope.joinofsevadata = response.data.movieArray;
    ...
    $scope.edit = function (y) {
        $scope.officebearersinformationid = y;
        window.location.href = "http://localhost:7900/bloodDonation/admin/officeBearersInformation?officebearersinformationid=" + $scope.officebearersinformationid;

    }


    $scope.oneDataDeleted = async (officebearersinformationid) => {
        try {
            await $http.post('http://localhost:7900/bloodDonation/adminRoute/officeBearersInformationDataDeleted', { officebearersinformationid: officebearersinformationid });
