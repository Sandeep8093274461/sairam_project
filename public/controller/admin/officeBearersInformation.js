...
app.controller("myCtrl", async ($scope, $http) => {
    ...
    $scope.loadSamithiList = async () => {
        try {
            let response = await $http.get('http://localhost:7900/bloodDonation/adminRoute/getresoursesDistWiseSamithiList?dist_id=' + $scope.district)
    ...
    $scope.loadDesignationList = async () => {
        try {
            let response = await $http.get('http://localhost:7900/bloodDonation/adminRoute/getDesignationList?lavel_id=' + $scope.level)
        ...
        if (passportPhoto != undefined) {
            ...
            request.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    // saiSabujeema
                    window.location.href = "http://localhost:7900/bloodDonation/admin/officeBearersInformation";
                }
            };
            data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
            request.open('POST', "http://localhost:7900/bloodDonation/adminRoute/addofficeBearersInformationData");
            request.send(data);
    ...
    $scope.edit = function () {
        if ($scope.officebearersinformationid != '') {
            $http.get("http://localhost:7900/bloodDonation/adminRoute/getOfficeBearersInformationDetails?officebearersinformationid=" + $scope.officebearersinformationid)
                .then(function (response) {
                    ...
                    $scope.movieArray = response.data.movieArray;
                    $scope.passportPhoto = $scope.details.passportphoto;

                    $scope.showPassportPhotoError = true;
                    $scope.PassportphotoFileGood = "UPLOADED";
                    $http.get("http://localhost:7900/bloodDonation/adminRoute/getLevel")
                        .then(function (response) {
                            $scope.levelList = response.data;
                            $scope.level = $scope.details.level;
                            $http.get('http://localhost:7900/bloodDonation/adminRoute/getDesignationList?lavel_id=' + $scope.level)
                                .then(function (response) {
                                    $scope.designationList = response.data;
                                    $scope.designation = $scope.details.designation;
                                });
                        ...
                        if ($scope.details.level == '02') {
                            ...
                            $http.get("http://localhost:7900/bloodDonation/adminRoute/getresoursesdistwisedata")
                                .then(function (response) {
                                    $scope.districtList = response.data;
                                    $scope.district = $scope.details.district;
                                    $http.get('http://localhost:7900/bloodDonation/adminRoute/getresoursesDistWiseSamithiList?dist_id=' + $scope.district)
                                        .then(function (response) {
                                            $scope.samithiList = response.data;
    ...
    $scope.update = async () => {
        ...
        $scope.data.profession = $scope.profession;
        $scope.data.mobNo = $scope.mobNo;
        if ($scope.wpNo == null || $scope.wpNo == undefined ||$scope.wpNo == '') {
            $scope.data.wpNo = '';
        }else{
            $scope.data.wpNo = $scope.wpNo;
        }
        ...
        $scope.data.email = $scope.email;
        if ( $scope.saiId == null || $scope.saiId == undefined ||$scope.saiId == '') {
            $scope.data.saiId = '';
        }else{
            $scope.data.saiId = $scope.saiId;
        }
        
        if ($scope.responsibility == null || $scope.responsibility == undefined ||$scope.responsibility == '' ) {
            $scope.data.responsibility = '';
        }else{
            $scope.data.responsibility = $scope.responsibility;
        }
        ...
        data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
        request.open('POST', "http://localhost:7900/bloodDonation/adminRoute/updateOfficeBearersInformationData");
        request.send(data);
