...
app.controller("myCtrl", async ($scope, $http) => {
    ...
    $scope.beneficiariesreportid = beneficiariesreportid;
    ...
    $scope.Submit = async () => {
        ...
        if (photo1 != undefined && photo2 != undefined) {
            ...
            $scope.data.dor = $scope.dor;
            $scope.data.nameOfbeneficiary = $scope.nameOfbeneficiary;
            $scope.data.type = $scope.type;
            $scope.data.area = $scope.area;
            $scope.data.sector = $scope.sector;
            $scope.data.specificScheme = $scope.specificScheme;
            $scope.data.actionDetails = $scope.actionDetails;
            $scope.data.approxExpenditureOrganisation = $scope.approxExpenditureOrganisation;
            $scope.data.approxExpenditureBeneficiary = $scope.approxExpenditureBeneficiary;
            $scope.data.monthlyIncomeAfterSwabalambi = $scope.monthlyIncomeAfterSwabalambi;
            $scope.data.bankLoan = $scope.bankLoan;
            $scope.data.noOfFamilyBenifited = $scope.noOfFamilyBenifited;
            ...
            request.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    // saiSabujeema
                    window.location.href = "http://localhost:7900/bloodDonation/admin/beneficiariesReport";
                }
            };
            data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
            request.open('POST', "http://localhost:7900/bloodDonation/adminRoute/addBeneficiariesReportaData");
            request.send(data);
    ...
    $scope.edit = function () {
        if ($scope.beneficiariesreportid != '') {
            $http.get("http://localhost:7900/bloodDonation/adminRoute/getBeneficiariesReportApplicationDetails?beneficiariesreportid=" + $scope.beneficiariesreportid)
                .then(function (response) {

                    $scope.details = response.data[0];
                    $scope.dor = new Date($scope.details.dor);
                    $scope.nameOfbeneficiary = $scope.details.nameofbeneficiary;
                    $scope.type = $scope.details.type;
                    $scope.area = $scope.details.area;
                    $scope.sector = $scope.details.sector;
                    $scope.specificScheme = $scope.details.specificscheme;
                    $scope.actionDetails = $scope.details.actiondetails;
                    $scope.approxExpenditureOrganisation = $scope.details.approxexpenditureorganisation;
                    $scope.approxExpenditureBeneficiary = $scope.details.approxexpenditurebeneficiary;
                    $scope.monthlyIncomeAfterSwabalambi = $scope.details.monthlyincomeafterswabalambi;
                    $scope.bankLoan = $scope.details.bankloan;
                    $scope.noOfFamilyBenifited = $scope.details.nooffamilybenifited;
    ...
    $scope.update = async () => {
        ...
        $scope.data.remarks = $scope.remarks;
        $scope.data.beneficiariesreportid = $scope.beneficiariesreportid;
        var data = new FormData();
        var request = new XMLHttpRequest();
        data.append('photo1', document.querySelector('#photo1').files[0]);
        data.append('photo2', document.querySelector('#photo2').files[0]);
        request.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                window.location.href = "http://localhost:7900/bloodDonation/admin/beneficiariesReportAllData";
            }
        };
        data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
        request.open('POST', "http://localhost:7900/bloodDonation/adminRoute/updateBeneficiariesReportData");
        request.send(data);
