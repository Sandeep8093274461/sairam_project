...
app.controller("myCtrl", async ($scope, $http) => {
    ...
    $scope.loadDistricts();
    $scope.validateSamiti = function () {
        $http.get("http://localhost:7900/bloodDonation/adminRoute/validateSamiti?newSamitiName=" + $scope.newSamitiName+ "&district_id=" + $scope.districtId.district_id)
            .then(function (response) {
                if (response.data == "") {
                    $scope.samithiId = false;
                }
                else {
                    $scope.samithiId = true;
                }
            });
    }

    $scope.addSamiti = async () => {
        
       
        $scope.data1 = [];
        $scope.data = {};
        $scope.data.district_id = $scope.districtId.district_id;
        $scope.data.newSamitiName = $scope.newSamitiName;
        try {
            await $http.post('http://localhost:7900/bloodDonation/adminRoute/addNewSamiti', $scope.data);
            window.alert(`New Samiti added Sucessfully`);
            window.location.href = "http://localhost:7900/bloodDonation/admin/addSamiti";
        } catch (e) {
