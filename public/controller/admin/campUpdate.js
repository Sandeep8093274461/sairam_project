...
app.controller("myCtrl", async ($scope, $http) => {
    ...
    $http.get('http://localhost:7900/bloodDonation/adminRoute/getConvenordatadistdistinct')
        .then(response => {
            ...
            if (campid != '') {
                $http.get("http://localhost:7900/bloodDonation/adminRoute/getCampById?campid=" + $scope.campid)
            ...
            // }
                ...
                request.onreadystatechange = function () {
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        window.location.href = "http://localhost:7900/bloodDonation/admin/campUpdate";
                    }
                };
                data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
                request.open('POST', "http://localhost:7900/bloodDonation/adminRoute/updateCampDetails1");
                request.send(data);
    ...
    $scope.addData = async () => {
        ...
        // if (parseInt(valFlag) == 1) {
            ...
            if (idp != undefined) {
                ...
                data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
                request.open('POST', "http://localhost:7900/bloodDonation/adminRoute/addCampDetails1");
                request.send(data);
