...
app.controller("myCtrl", async ($scope, $http, Excel, $timeout) => {
    ...
    $scope.getFilteredData = async () => {
        ...
        $scope.totalNosdi = 0;
        $scope.totalAppCost = 0;
        $http.post('http://localhost:7900/bloodDonation/adminRoute/getFilteredConvenorData', { dist_id: $scope.dist, fromDate: $scope.fromDate, toDate: $scope.toDate })
            .then(response => {
    ...
    $scope.editField = (y) => {
        $scope.conv_id = y;
        window.location.href = "http://localhost:7900/bloodDonation/admin/convenorDetails?conv_id="+$scope.conv_id;
    ...
    $scope.deleteField = (cmp) => {
        ...
        $http.get('http://localhost:7900/bloodDonation/adminRoute/getConvenorDetails/' + un)
            .then(response => {
                $scope.indexConvenorList = response.data;
            })
    }
    ...
    $scope.removeSevaData = async () => {
        let response = await $http.post('http://localhost:7900/bloodDonation/adminRoute/removeSevaData/' + $scope.removeItem.conv_id)
        if (response.data) {
