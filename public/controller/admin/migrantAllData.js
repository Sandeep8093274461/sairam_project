...
app.controller("myCtrl", async ($scope, $http) => {
    ...
    // ng-repeat="data in results =  (alldata | dateRangeFrom : fromDateFilter : this | dateRangeTo : toDateFilter:this | filter: { returningstate: fromStateFilter.statecode }  | filter: { district:fromDistrictFilter } | filter:(!!fromGenderFilter || undefined) && fromGenderFilter:true | filter:(!!fromStatusFilter || undefined) && fromStatusFilter:true | filter:(!!fromSkillFilter || undefined) && fromSkillFilter:true )">
    $scope.allDataShow = function () {
        try {
            $http.get('http://localhost:7900/bloodDonation/adminRoute/allSkillDataShow?district_id=' + district_id+ "&user_id=" + user_id)
                .then(function (response) {
                    $scope.alldata = response.data.details;
                    $scope.skillList = response.data.skilldetails;
                });
    ...
    $scope.view = function (y) {
        $scope.migrate_id = y;
        window.location.href = "http://localhost:7900/bloodDonation/admin/migrantSurveyViewPage?migrate_id=" + $scope.migrate_id;
    }
    $scope.edit = function (y) {
        $scope.migrate_id = y;
        
        window.location.href = "http://localhost:7900/bloodDonation/admin/migrantSurvey?migrate_id=" + $scope.migrate_id;

    }
    // $scope.skillCheck = function () {
      
    //    try {
    //     await $http.post('http://localhost:7900/bloodDonation/skillCheck');
    //     $scope.allDataShow();
    ...
    $scope.oneDataDeleted = async (migrate_id) => {
        try {
            await $http.post('http://localhost:7900/bloodDonation/adminRoute/oneDataDeleted', { migrate_id: migrate_id });
            $scope.allDataShow();
