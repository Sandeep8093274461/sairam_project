...
app.controller("myCtrl", async ($scope, $http, Excel, $timeout) => {
    ...
    $scope.getFilteredData = async () => {
        if ($scope.fromDate != undefined && $scope.toDate != undefined) {
            $http.post('http://localhost:7900/bloodDonation/userRoute/activitiesReportAllData', { dist_id: $scope.dist, fromDate: $scope.fromDate, toDate: $scope.toDate, district_id: district_id, user_id: $scope.user_id })
                .then(response => {
    ...
    $scope.view = function (y) {
        ...
        try {
            $http.get('http://localhost:7900/bloodDonation/userRoute/activitiesReporttDatashow?activitiesreportid=' +  $scope.activitiesreportid)
                .then(function (response) {
    ...
    $scope.edit = function (y) {
        $scope.activitiesreportid = y;
        window.location.href = "http://localhost:7900/bloodDonation/user/activitiesReport?activitiesreportid=" + $scope.activitiesreportid;
    ...
    $scope.oneDataDeleted = async (activitiesreportid) => {
        try {
            await $http.post('http://localhost:7900/bloodDonation/userRoute/activitiesReportDataDeleted', { activitiesreportid: activitiesreportid });
