...
app.controller("myCtrl", async ($scope, $http) => {
    ...
    $scope.allDataShow = function () {
        try {
            $http.get('http://localhost:7900/bloodDonation/userRoute/allSkillDataShow?district_id=' + district_id)
                .then(function (response) {
    ...
    $scope.oneDataDeleted = async (migrate_id) => {
        try {
            await $http.post('http://localhost:7900/bloodDonation/userRoute/oneDataDeleted', { migrate_id: migrate_id });
            $scope.allDataShow();
