...
app.controller("myCtrl", async ($scope, $http) => {
        ...
        if (idp != undefined) {
            ...
            request.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    window.location.href = "http://localhost:7900/bloodDonation/user/saiSabujeema";
                }
            };
            data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
            request.open('POST', "http://localhost:7900/bloodDonation/userRoute/addSaiSabuJeemaData");
            request.send(data);
    ...
    $scope.edit = function () {
        if ($scope.sabujeema_id != '') {
            $http.get("http://localhost:7900/bloodDonation/userRoute/getSaiSabujeemaApplicationDetails?sabujeema_id=" + $scope.sabujeema_id)
                .then(function (response) {
    ...
    $scope.update = async () => {
        ...
        data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
        request.open('POST', "http://localhost:7900/bloodDonation/userRoute/updateSaiSabuJeemaData");
        request.send(data);
