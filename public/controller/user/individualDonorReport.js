...
app.controller("myCtrl", async ($scope, $http,Excel, $timeout) => {
    ...
    $scope.allDataShow = function () {
        if ($scope.fromDate != undefined && $scope.toDate != undefined) {
            try {
                $http.post('http://localhost:7900/bloodDonation/adminRoute/getIndividualDonorDetails', {district: $scope.user_id,bg: $scope.bgFilter, fromDate: $scope.fromDate, toDate: $scope.toDate, user_id: $scope.user_id })
                    .then(response => {
