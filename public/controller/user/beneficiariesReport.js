...
app.controller("myCtrl", async ($scope, $http) => {
        ...
        if (photo1 != undefined && photo2 != undefined) {
            ...
            request.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    // saiSabujeema
                    window.location.href = "http://localhost:7900/bloodDonation/user/beneficiariesReport";
                }
            };
            data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
            request.open('POST', "http://localhost:7900/bloodDonation/userRoute/addBeneficiariesReportaData");
            request.send(data);
    ...
    $scope.edit = function () {
        if ($scope.beneficiariesreportid != '') {
            $http.get("http://localhost:7900/bloodDonation/userRoute/getBeneficiariesReportApplicationDetails?beneficiariesreportid=" + $scope.beneficiariesreportid)
                .then(function (response) {
    ...
    $scope.update = async () => {
        ...
        request.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                window.location.href = "http://localhost:7900/bloodDonation/user/beneficiariesReportAllData";
            }
        };
        data.append('Name1', JSON.stringify(JSON.parse(angular.toJson($scope.data))));
        request.open('POST', "http://localhost:7900/bloodDonation/userRoute/updateBeneficiariesReportData");
        request.send(data);
