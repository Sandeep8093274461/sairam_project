...
app.controller("myCtrl", async ($scope, $http,Excel, $timeout) => {
    ...
    $scope.allDataShow = function () {
        try {
            $http.get('http://localhost:7900/bloodDonation/userRoute/allResourseDataShow?district_id=' + district_id+ "&user_id=" + $scope.user_id)
                .then(function (response) {
    ...
    $scope.oneDataDeleted = async (id) => {
        try {
            await $http.post('http://localhost:7900/bloodDonation/userRoute/oneResourceDataDeleted', { id: id });
            $scope.allDataShow();
