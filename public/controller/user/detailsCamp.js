...
app.controller("myCtrl", async ($scope, $http,Excel, $timeout) => {
    ...
    $scope.getFilteredData = function (y) {
        ...
        $http.post('http://localhost:7900/bloodDonation/userRoute/getCmpDetails',{fromDate: $scope.fromDate, toDate: $scope.toDate, user_id: $scope.user_id })
        .then(response => {
    ...
    $scope.editField = function (y) {
        $scope.campid = y;
        window.location.href = "http://localhost:7900/bloodDonation/user/campUpdate?campid=" + $scope.campid;
    ...
    $scope.deleteField = async (campid) => {
        try {
            await $http.post('http://localhost:7900/bloodDonation/userRoute/campUpdateDataDeleted', { campid: campid });
