...
app.controller("myCtrl", async ($scope, $http,Excel, $timeout) => {
    ...
    $scope.loadFilterDistricts();
    $scope.viewFilterData = () =>  {
        try {
            if ($scope.bg != undefined && $scope.status != undefined) {
            $http.post('http://localhost:7900/bloodDonation/userRoute/getDonorFilterList', { bg: $scope.bg, district: user_id, status: $scope.status })
    ...
    // $http.get('http://localhost:7900/bloodDonation/userRoute/getCmpDetails')
    //     .then(response => {
    ...
    $scope.exportToExcel = function (tableIdSeed) {
        var exportHref = Excel.tableToExcel(tableIdSeed, 'donorsearch');
