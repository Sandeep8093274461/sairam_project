...
app.controller("myCtrl", async ($scope, $http, Excel, $timeout) => {
    ...
    $scope.loadSamithiList = async () => {
        try {
            let response = await $http.get('http://localhost:7900/bloodDonation/userRoute/getresoursesDistWiseSamithiList?dist_id=' + $scope.fromDistrictFilter)
    ...
    $scope.bhajanMandaliList = async () => {
        try {
            let response = await $http.get('http://localhost:7900/bloodDonation/userRoute/bhajanMandaliListwiseData?samithi_id=' + $scope.fromSamitiFilter)
    ...
    $scope.getFilteredData = async () => {
        if ($scope.fromDate != undefined && $scope.toDate != undefined) {
            $http.post('http://localhost:7900/bloodDonation/userRoute/saiSabujeemaAllDataShow', { dist_id: $scope.dist, fromDate: $scope.fromDate, toDate: $scope.toDate, district_id: district_id, user_id: $scope.user_id })
                .then(response => {
    ...
    $scope.view = function (y) {
        $scope.sabujeema_id = y;
        try {
            $http.get('http://localhost:7900/bloodDonation/userRoute/saiSabujeemaDatashow?sabujeema_id=' + $scope.sabujeema_id)
                .then(function (response) {
    ...
    $scope.edit = function (y) {
        $scope.sabujeema_id = y;
        window.location.href = "http://localhost:7900/bloodDonation/user/saiSabujeema?sabujeema_id=" + $scope.sabujeema_id;
    ...
    $scope.oneDataDeleted = async (sabujeema_id) => {
        try {
            await $http.post('http://localhost:7900/bloodDonation/userRoute/saiSabujeemaDataDeleted', { sabujeema_id: sabujeema_id });
    ...
    $scope.exportToExcel = function (tableIdSeed) {
        var exportHref = Excel.tableToExcel(tableIdSeed, 'saiSabujeemadata');
...
function loadPage(content, callback) {
    ...
    mywindow.document.write('<html><head><title>Sai Sabujeema</title><meta name="user_id" content="print">');
